...
% load('cylinder_simulation_data.mat','U','dU', 't', 'outer_residual', 'n');
% U_old = U;
...
while outer_residual >= 10e-6
    ...
    if abs(mod(n, 2)) == 0
        ...
        % save('cylinder_simulation_data.mat', 'U', 'dU', 't', 'n', 'outer_residual')   
    ...
    while global_inner_residual >= 10e-7
        ...
        for i = 1:Nx
            ...
            elseif i > 1 && i < Nx
                ...
                dU_new(:,i,:) = Inner_Tridiagonal_Solver(dU_old, U, U_inlet, Ny, S, V, Face_Normal, dt, i);
                global_inner_residual = global_inner_residual + (calculateLocalResidual(dU_new, dU_old, U, V, dt, Ny, S, Face_Normal, i))^2;

            elseif i == Nx

                dU_new(:,Nx,:) = Right_Boundary_Tridiagonal_Solver(dU_old, U, U_inlet, Ny, Nx, S, V, Face_Normal, dt);               
...
function[dU_new] = Left_Boundary_Tridiagonal_Solver(dU_old, U, U_inlet, Ny, S, V, Face_Normal, dt)
    ...
    Ugo = U(:,1,Ny);
    a =  V(1,Ny)/dt*eye(4) ...
             + ( AP(U(:,1,Ny), Ugo, Face_Normal(1,1,1,Ny), Face_Normal(2,1,1,Ny)) + Eo*AM(U(:,1,Ny), Ugo, Face_Normal(1,1,1,Ny), Face_Normal(2,1,1,Ny)) )*S(1,1,Ny) ...   % Left (Outlet BC)
             ...
             + (AP(U(:,1,Ny), Ugi, Face_Normal(1,4,1,Ny), Face_Normal(2,4,1,Ny)) + Ei*AM(U(:,1,Ny), Ugi, Face_Normal(1,4,1,Ny), Face_Normal(2,4,1,Ny)))*S(4,1,Ny);        % Top (Inlet BC) 
    
    c = AM(U(:,1,Ny), U(:,1,Ny-1), Face_Normal(1,3,1,Ny), Face_Normal(2,3,1,Ny))*S(3,1,Ny);
    
    f =  -(   ( AP(U(:,1,Ny), Ugo, Face_Normal(1,1,1,Ny), Face_Normal(2,1,1,Ny))*U(:,1,Ny) + AM(U(:,1,Ny), Ugo, Face_Normal(1,1,1,Ny), Face_Normal(2,1,1,Ny))*Ugo )*S(1,1,Ny)...                            % Left (Outlet BC)
            + ( AP(U(:,1,Ny), U(:,2,Ny), Face_Normal(1,2,1,Ny), Face_Normal(2,2,1,Ny))*U(:,1,Ny) + AM(U(:,1,Ny), U(:,2,Ny), Face_Normal(1,2,1,Ny), Face_Normal(2,2,1,Ny))*U(:,2,Ny) )*S(2,1,Ny)...          % Right
            + ( AP(U(:,1,Ny), U(:,1,Ny-1), Face_Normal(1,3,1,Ny), Face_Normal(2,3,1,Ny))*U(:,1,Ny) + AM(U(:,1,Ny), U(:,1,Ny-1), Face_Normal(1,3,1,Ny), Face_Normal(2,3,1,Ny))*U(:,1,Ny-1) )*S(3,1,Ny)...    % Bottom 
            + ( AP(U(:,1,Ny), Ugi, Face_Normal(1,4,1,Ny), Face_Normal(2,4,1,Ny))*U(:,1,Ny) + AM(U(:,1,Ny), Ugi, Face_Normal(1,4,1,Ny), Face_Normal(2,4,1,Ny))*Ugi )*S(4,1,Ny)...                            % Top (Inlet BC)    
    ...
    for j = (Ny-1):-1:2
        ...
        a =  V(1,j)/dt*eye(4) ...
             + ( AP(U(:,1,j), Ugo, Face_Normal(1,1,1,j), Face_Normal(2,1,1,j)) + Eo*AM(U(:,1,j), Ugo, Face_Normal(1,1,1,j), Face_Normal(2,1,1,j)) )*S(1,1,j) ...   % Left (Outlet BC)
        ...
        f =  -(   ( AP(U(:,1,j), Ugo, Face_Normal(1,1,1,j), Face_Normal(2,1,1,j))*U(:,1,j) + AM(U(:,1,j), Ugo, Face_Normal(1,1,1,j), Face_Normal(2,1,1,j))*Ugo )*S(1,1,j)...                         % Left (Outlet BC)
                + ( AP(U(:,1,j), U(:,2,j), Face_Normal(1,2,1,j), Face_Normal(2,2,1,j))*U(:,1,j) + AM(U(:,1,j), U(:,2,j), Face_Normal(1,2,1,j), Face_Normal(2,2,1,j))*U(:,2,j) )*S(2,1,j)...          % Right 
                + ( AP(U(:,1,j), U(:,1,j-1), Face_Normal(1,3,1,j), Face_Normal(2,3,1,j))*U(:,1,j) + AM(U(:,1,j), U(:,1,j-1), Face_Normal(1,3,1,j), Face_Normal(2,3,1,j))*U(:,1,j-1) )*S(3,1,j)...    % Bottom
                + ( AP(U(:,1,j), U(:,1,j+1), Face_Normal(1,4,1,j), Face_Normal(2,4,1,j))*U(:,1,j) + AM(U(:,1,j), U(:,1,j+1), Face_Normal(1,4,1,j), Face_Normal(2,4,1,j))*U(:,1,j+1) )*S(4,1,j) ...   % Top               
    ...
    a =  V(1,1)/dt*eye(4) ... 
             + ( AP(U(:,1,1), Ugo, Face_Normal(1,1,1,1), Face_Normal(2,1,1,1)) + Eo*AM(U(:,1,1), Ugo, Face_Normal(1,1,1,1), Face_Normal(2,1,1,1)) )*S(1,1,1) ...    % Left (Inlet BC)
    ...
    f =  -(   ( AP(U(:,1,1), Ugo, Face_Normal(1,1,1,1), Face_Normal(2,1,1,1))*U(:,1,1) + AM(U(:,1,1), Ugo, Face_Normal(1,1,1,1), Face_Normal(2,1,1,1))*Ugo )*S(1,1,1)...                           % Left (Inlet BC at time n)
            + ( AP(U(:,1,1), U(:,2,1), Face_Normal(1,2,1,1), Face_Normal(2,2,1,1))*U(:,1,1) + AM(U(:,1,1), U(:,2,1), Face_Normal(1,2,1,1), Face_Normal(2,2,1,1))*U(:,2,1) )*S(2,1,1)...            % Right (time n)
            + ( AP(U(:,1,1), Ugbw, Face_Normal(1,3,1,1), Face_Normal(2,3,1,1))*U(:,1,1) + AM(U(:,1,1), Ugbw, Face_Normal(1,3,1,1), Face_Normal(2,3,1,1))*Ugbw )*S(3,1,1)...                        % Bottom (time n) (Wall BC at time n)
            + ( AP(U(:,1,1), U(:,1,2), Face_Normal(1,4,1,1), Face_Normal(2,4,1,1))*U(:,1,1) + AM(U(:,1,1), U(:,1,2), Face_Normal(1,4,1,1), Face_Normal(2,4,1,1))*U(:,1,2) )*S(4,1,1) ...           % Top      
...
function[dU_new] = Inner_Tridiagonal_Solver(dU_old, U, U_inlet, Ny, S, V, Face_Normal, dt, i)
    ...
    a = V(i,Ny)/dt*eye(4) ...
                    + AP(U(:,i,Ny), U(:,i-1,Ny), Face_Normal(1,1,i,Ny), Face_Normal(2,1,i,Ny))*S(1,1,Ny) ...                                                    % Left
    ...
    a = V(i,1)/dt*eye(4) ... 
                ...
                + AP(U(:,i,1), U(:,i,2), Face_Normal(1,4,i,1), Face_Normal(2,4,i,1))*S(4,i,1);                                                                              % Top 
    
    f = -( ( AP(U(:,i,1), U(:,i-1,1), Face_Normal(1,1,i,1), Face_Normal(2,1,i,1))*U(:,i,1) + AM(U(:,i,1), U(:,i-1,1), Face_Normal(1,1,i,1), Face_Normal(2,1,i,1))*U(:,i-1,1) )*S(1,i,1)...        % Left (time n)
         + ( AP(U(:,i,1), U(:,i+1,1), Face_Normal(1,2,i,1), Face_Normal(2,2,i,1))*U(:,i,1) + AM(U(:,i,1), U(:,i+1,1), Face_Normal(1,2,i,1), Face_Normal(2,2,i,1))*U(:,i+1,1) )*S(2,i,1)...        % Right (time n)
         + ( AP(U(:,i,1), Ugbw, Face_Normal(1,3,i,1), Face_Normal(2,3,i,1))*U(:,i,1) + AM(U(:,i,1), Ugbw, Face_Normal(1,3,i,1), Face_Normal(2,3,i,1))*Ugbw )*S(3,i,1)...                        % Bottom (Wall BC at time n)
         + ( AP(U(:,i,1), U(:,i,2), Face_Normal(1,4,i,1), Face_Normal(2,4,i,1))*U(:,i,1) + AM(U(:,i,1), U(:,i,2), Face_Normal(1,4,i,1), Face_Normal(2,4,i,1))*U(:,i,2) )*S(4,i,1)...            % Top (time n)            
...
function[dU] = Right_Boundary_Tridiagonal_Solver(dU_old, U, U_inlet, Ny, Nx, S, V, Face_Normal, dt)
    ...
    [Ugbw, Ewb] = Wallghost(U(:,Nx,Ny), Face_Normal(1,3,Nx,1), Face_Normal(2,3,Nx,1));
    Ugo = U(:,Nx,1);
